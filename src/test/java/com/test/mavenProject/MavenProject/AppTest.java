...
public class AppTest extends Generic
{
    ...
    @AfterMethod
    public void jiraTest(ITestResult result) {
    	if(result.getStatus()==ITestResult.FAILURE) {
    		BasicCredentials creds= new BasicCredentials("bharathkalapakuri@hotmail.com","H5LvILUVjCg5YQuJtgua4465");
    		JiraClient jira = new JiraClient("https://seleniumjenkins.atlassian.net/", creds);
    		try {
				...
				//SearchResult searchResult=jira.searchIssues("project = \"MavenProject\" AND fixVersion = \"Unscheduled\" AND cycleName = \"MavenProject\"");
				...
				//CZApiCycleClient zapi = new CZApiCycleClient();
				//zapi.getCycleById(1);
				//RestClient restClient = new RestClient(null, creds, null)
				//zapi.getExecutions("MAV");
    ...
    public void jiraTest() {
    	   System.out.println("Welcome to no of test cases in test cycle::");
    		...
    		try {
				...
				//issue.
				//List<String> str=(List<String>) jira.getRestClient().post("https://seleniumjenkins.atlassian.net/rest/zapi/latest/execution?projectId=\"MAV\"");
    ...
    @Test
    public void zapiTest() {
    	System.out.println("Zapi actions");
    	
    	
    	
    	//CZApiExecutions exe= CZApiClient.Search.getExecutions("project = \"MavenProject\" AND fixVersion = \"Unscheduled\" AND cycleName = \"MavenProject\"");
